...
#define darr(n,t) _d_darr_new((n),sizeof(t))
#define darr_free(a) { _d_free(a->data); _d_free(a); }

/* Define initializer and destructor */

static inline darr_t _d_darr_new(size_t nmemb, size_t esize) {
...
/* for access we like lvalue type access, instead of set, get: e.g. x
   = val(int,a,i) or val(int,a,i) = x in this case len(a) has to give
   last accessed element, not necessarily last set.  this risks the
   user to accidentally allocate a huge array, oh well. */

#define val(a,i,t) (*((t*)_d_boundcheck((a),(i),sizeof(t))))
